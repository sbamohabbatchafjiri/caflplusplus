...
static u8 could_be_arith(u32 old_val, u32 new_val, u8 blen, u8 use_feistel) {
  ...
  /* See if two-byte adjustments could produce this result. */
  ...
  if (diffs == 1) {
    ...
    /* Conditionally use Feistel or simple swap */
    if (use_feistel) {
      ov = FEISTEL_SWAP16(ov);
      nv = FEISTEL_SWAP16(nv);
    } else {
      ov = SWAP16(ov);
      nv = SWAP16(nv);
    }
  ...
  if (blen == 4) {
    ...
    if (use_feistel) {
      old_val = FEISTEL_SWAP32(old_val);
      new_val = FEISTEL_SWAP32(new_val);
    } else {
